...
func = lambda logp: np.array(model0_fit.m.res_log_params(logp))
jacobian = lambda logp: np.array(model0_fit.m.jacobian_log_params_sens(logp).values())
...
j = model0_fit.m.jacobian_log_params_sens(geo.xs[-1,:])
jtj = np.dot(np.transpose(j), j)
e, v = Utility.eig(jtj)
e = scipy.real(e)
Plotting.figure(3)
...
print(SDisc)
print('corresponding to %d',model0_fit.popt.items()[ SDisc.items()[0][0]])
