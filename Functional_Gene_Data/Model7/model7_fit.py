...
model_net = IO.from_SBML_file('PECCAT7.xml', 'base')
...
IO.eqns_TeX_file(model_net, 'model7.tex')

## Create the model
m = Model([PECCAT_experiment.expt], [model_net])
p0= m.get_params().copy()

p7 = Utility.load('p6.geodesics.bpkl')
p7 = np.delete(p7, 6)
...
res = Residuals.PriorInLog('YLHiq_prior', 'YLHiq', np.log(p0[37]), np.log(np.sqrt(p0[37])))
m.AddResidual(res)
res = Residuals.PriorInLog('YLLoq_prior', 'YLLoq', np.log(p0[38]), np.log(np.sqrt(p0[38])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrB_prior', 'YrB', np.log(p0[39]), np.log(np.sqrt(p0[39])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrF_prior', 'YrF', np.log(p0[40]), np.log(np.sqrt(p0[40])))
m.AddResidual(res)
res = Residuals.PriorInLog('YRFP_prior', 'YRFP', np.log(p0[41]), np.log(np.sqrt(p0[41])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBHiq_prior', 'YsBHiq', np.log(p0[42]), np.log(np.sqrt(p0[42])))
m.AddResidual(res)#
res = Residuals.PriorInLog('YsBLoq_prior', 'YsBLoq', np.log(p0[43]), np.log(np.sqrt(p0[43])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPHiq_prior', 'YsBPHiq', np.log(p0[44]), np.log(np.sqrt(p0[44])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPLoq_prior', 'YsBPLoq', np.log(p0[45]), np.log(np.sqrt(p0[45])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFHiq_prior', 'YsFHiq', np.log(p0[46]), np.log(np.sqrt(p0[46])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFLoq_prior', 'YsFLoq', np.log(p0[47]), np.log(np.sqrt(p0[47])))
m.AddResidual(res)
res = Residuals.PriorInLog('rB0_prior', 'rB0', np.log(p0[48]), np.log(np.sqrt(p0[48])))
m.AddResidual(res)
res = Residuals.PriorInLog('rBP0_prior', 'rBP0', np.log(p0[49]), np.log(np.sqrt(p0[49])))
m.AddResidual(res)
res = Residuals.PriorInLog('rF0_prior', 'rF0', np.log(p0[50]), np.log(np.sqrt(p0[50])))
m.AddResidual(res)
...
Utility.save(popt, 'popt7.model.bpkl')
