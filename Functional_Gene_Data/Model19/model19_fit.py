...
model_net = IO.from_SBML_file('PECCAT19.xml', 'base')
...
IO.eqns_TeX_file(model_net, 'model18.tex')

## Create the model
m = Model([PECCAT_experiment.expt], [model_net])
p0= m.get_params().copy()

p19 = Utility.load('p18.geodesics.bpkl')
p19 = np.delete(p19,36)
p19 = np.delete(p19,35)
p19 = np.delete(p19,33)
p19 = np.delete(p19,24)
...
res = Residuals.PriorInLog('YLHiq_prior', 'YLHiq', np.log(p0[25]), np.log(np.sqrt(p0[25])))
m.AddResidual(res)
res = Residuals.PriorInLog('YLLoq_prior', 'YLLoq', np.log(p0[26]), np.log(np.sqrt(p0[26])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrB_prior', 'YrB', np.log(p0[27]), np.log(np.sqrt(p0[27])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrF_prior', 'YrF', np.log(p0[28]), np.log(np.sqrt(p0[28])))
m.AddResidual(res)
res = Residuals.PriorInLog('YRFP_prior', 'YRFP', np.log(p0[29]), np.log(np.sqrt(p0[29])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBHiq_prior', 'YsBHiq', np.log(p0[30]), np.log(np.sqrt(p0[30])))
m.AddResidual(res)#
res = Residuals.PriorInLog('YsBLoq_prior', 'YsBLoq', np.log(p0[31]), np.log(np.sqrt(p0[31])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPLoq_prior', 'YsBPLoq', np.log(p0[32]), np.log(np.sqrt(p0[32])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFLoq_prior', 'YsFLoq', np.log(p0[33]), np.log(np.sqrt(p0[33])))
m.AddResidual(res)
res = Residuals.PriorInLog('rB0_prior', 'rB0', np.log(p0[34]), np.log(np.sqrt(p0[34])))
m.AddResidual(res)
res = Residuals.PriorInLog('rBP0_prior', 'rBP0', np.log(p0[35]), np.log(np.sqrt(p0[35])))
m.AddResidual(res)
res = Residuals.PriorInLog('rF0_prior', 'rF0', np.log(p0[36]), np.log(np.sqrt(p0[36])))
m.AddResidual(res)
...
Utility.save(popt, 'popt19.model.bpkl')
