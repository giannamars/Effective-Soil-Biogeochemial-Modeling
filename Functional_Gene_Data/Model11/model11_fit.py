...
model_net = IO.from_SBML_file('PECCAT11.xml', 'base')
...
IO.eqns_TeX_file(model_net, 'model11.tex')

## Create the model
m = Model([PECCAT_experiment.expt], [model_net])
p0= m.get_params().copy()

p11 = Utility.load('p10.geodesics.bpkl')
p11 = np.delete(p11, 43)
p11 = np.delete(p11, 41)
p11 = np.delete(p11, 17)
...
res = Residuals.PriorInLog('YLHiq_prior', 'YLHiq', np.log(p0[33]), np.log(np.sqrt(p0[33])))
m.AddResidual(res)
res = Residuals.PriorInLog('YLLoq_prior', 'YLLoq', np.log(p0[34]), np.log(np.sqrt(p0[34])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrB_prior', 'YrB', np.log(p0[35]), np.log(np.sqrt(p0[35])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrF_prior', 'YrF', np.log(p0[36]), np.log(np.sqrt(p0[36])))
m.AddResidual(res)
res = Residuals.PriorInLog('YRFP_prior', 'YRFP', np.log(p0[37]), np.log(np.sqrt(p0[37])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBHiq_prior', 'YsBHiq', np.log(p0[38]), np.log(np.sqrt(p0[38])))
m.AddResidual(res)#
res = Residuals.PriorInLog('YsBLoq_prior', 'YsBLoq', np.log(p0[39]), np.log(np.sqrt(p0[39])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPLoq_prior', 'YsBPLoq', np.log(p0[40]), np.log(np.sqrt(p0[40])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFHiq_prior', 'YsFHiq', np.log(p0[41]), np.log(np.sqrt(p0[41])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFLoq_prior', 'YsFLoq', np.log(p0[42]), np.log(np.sqrt(p0[42])))
m.AddResidual(res)
res = Residuals.PriorInLog('rB0_prior', 'rB0', np.log(p0[43]), np.log(np.sqrt(p0[43])))
m.AddResidual(res)
res = Residuals.PriorInLog('rBP0_prior', 'rBP0', np.log(p0[44]), np.log(np.sqrt(p0[44])))
m.AddResidual(res)
res = Residuals.PriorInLog('rF0_prior', 'rF0', np.log(p0[45]), np.log(np.sqrt(p0[45])))
m.AddResidual(res)

## Optimize to fit data
print 'Initial Cost:', m.cost(p0)
popt = Optimization.fmin_lm_log_params(m, p0, maxiter=2, disp=True)
...
Utility.save(popt, 'popt11.model.bpkl')
