...
model_net = IO.from_SBML_file('PECCAT30.xml', 'base')
...
IO.eqns_TeX_file(model_net, 'model30.tex')

## Create the model
m = Model([PECCAT_experiment.expt], [model_net])
p0= m.get_params().copy()

p30 = Utility.load('p29.geodesics.bpkl')
#
p30[9]=12
p30[10]=476
p30[15]=312
#
p30 = np.delete(p30,29)
p30 = np.delete(p30,27)
p30 = np.delete(p30,26)
p30 = np.delete(p30,24)
p30 = np.delete(p30,4)
...
res = Residuals.PriorInLog('YLHiq_prior', 'YLHiq', np.log(p0[16]), np.log(np.sqrt(p0[16])))
m.AddResidual(res)
res = Residuals.PriorInLog('YLLoq_prior', 'YLLoq', np.log(p0[17]), np.log(np.sqrt(p0[17])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrB_prior', 'YrB', np.log(p0[18]), np.log(np.sqrt(p0[18])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrF_prior', 'YrF', np.log(p0[19]), np.log(np.sqrt(p0[19])))
m.AddResidual(res)
res = Residuals.PriorInLog('YRFP_prior', 'YRFP', np.log(p0[20]), np.log(np.sqrt(p0[20])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBHiq_prior', 'YsBHiq', np.log(p0[21]), np.log(np.sqrt(p0[21])))
m.AddResidual(res)#
res = Residuals.PriorInLog('YsBLoq_prior', 'YsBLoq', np.log(p0[22]), np.log(np.sqrt(p0[22])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPLoq_prior', 'YsBPLoq', np.log(p0[23]), np.log(np.sqrt(p0[23])))
m.AddResidual(res)
...
Utility.save(popt, 'popt30.model.bpkl')
