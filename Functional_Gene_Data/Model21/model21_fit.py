...
model_net = IO.from_SBML_file('PECCAT21.xml', 'base')
...
IO.eqns_TeX_file(model_net, 'model20.tex')

## Create the model
m = Model([PECCAT_experiment.expt], [model_net])
p0= m.get_params().copy()

p21 = Utility.load('p20.geodesics.bpkl')
p21[22] = 0.001
p21 = np.delete(p21,34)
p21 = np.delete(p21,33)
p21 = np.delete(p21,31)
p21 = np.delete(p21,13)
...
res = Residuals.PriorInLog('YLHiq_prior', 'YLHiq', np.log(p0[23]), np.log(np.sqrt(p0[23])))
m.AddResidual(res)
res = Residuals.PriorInLog('YLLoq_prior', 'YLLoq', np.log(p0[24]), np.log(np.sqrt(p0[24])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrB_prior', 'YrB', np.log(p0[25]), np.log(np.sqrt(p0[25])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrF_prior', 'YrF', np.log(p0[26]), np.log(np.sqrt(p0[26])))
m.AddResidual(res)
res = Residuals.PriorInLog('YRFP_prior', 'YRFP', np.log(p0[27]), np.log(np.sqrt(p0[27])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBHiq_prior', 'YsBHiq', np.log(p0[28]), np.log(np.sqrt(p0[28])))
m.AddResidual(res)#
res = Residuals.PriorInLog('YsBLoq_prior', 'YsBLoq', np.log(p0[29]), np.log(np.sqrt(p0[29])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPLoq_prior', 'YsBPLoq', np.log(p0[30]), np.log(np.sqrt(p0[30])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFLoq_prior', 'YsFLoq', np.log(p0[31]), np.log(np.sqrt(p0[31])))
m.AddResidual(res)
res = Residuals.PriorInLog('rB0_prior', 'rB0', np.log(p0[32]), np.log(np.sqrt(p0[32])))
m.AddResidual(res)
res = Residuals.PriorInLog('rBP0_prior', 'rBP0', np.log(p0[33]), np.log(np.sqrt(p0[33])))
m.AddResidual(res)
res = Residuals.PriorInLog('rF0_prior', 'rF0', np.log(p0[34]), np.log(np.sqrt(p0[34])))
m.AddResidual(res)
...
Utility.save(popt, 'popt21.model.bpkl')
