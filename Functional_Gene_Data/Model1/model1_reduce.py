...
x = np.log(model1_fit.popt)

# Calculate jtj approximation to hessian (in log params) and plot eigenvalue
# spectrum
j = model1_fit.m.jacobian_log_params_sens(np.log(model1_fit.popt))
...
func = lambda logp: np.array(model1_fit.m.res_log_params(logp))
jacobian = lambda logp: np.array(model1_fit.m.jacobian_log_params_sens(logp).values())
...
# Calculate jtj approximation to hessian (in log params) and plot eigenvalue
# spectrum
j = model1_fit.m.jacobian_log_params_sens(geo.xs[-1,:])
jtj = np.dot(np.transpose(j), j)
e, v = Utility.eig(jtj)
e = scipy.real(e)
Plotting.figure(3)
...
print(SDisc)
print('corresponding to %d',model1_fit.popt.items()[ SDisc.items()[0][0]])
