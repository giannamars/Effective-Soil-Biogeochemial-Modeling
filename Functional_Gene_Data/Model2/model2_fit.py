...
model_net = IO.from_SBML_file('PECCAT2.xml', 'base')
...
IO.eqns_TeX_file(model_net, 'model2.tex')

## Create the model
m = Model([PECCAT_experiment.expt], [model_net])
p0= m.get_params().copy()

p2 = Utility.load('p1.geodesics.bpkl')
p2 = np.delete(p2, 25)
...
res = Residuals.PriorInLog('YLHiq_prior', 'YLHiq', np.log(p0[42]), np.log(np.sqrt(p0[42])))
m.AddResidual(res)
res = Residuals.PriorInLog('YLLoq_prior', 'YLLoq', np.log(p0[43]), np.log(np.sqrt(p0[43])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrB_prior', 'YrB', np.log(p0[44]), np.log(np.sqrt(p0[44])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrF_prior', 'YrF', np.log(p0[45]), np.log(np.sqrt(p0[45])))
m.AddResidual(res)
res = Residuals.PriorInLog('YRFP_prior', 'YRFP', np.log(p0[46]), np.log(np.sqrt(p0[46])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBHiq_prior', 'YsBHiq', np.log(p0[47]), np.log(np.sqrt(p0[47])))
m.AddResidual(res)#
res = Residuals.PriorInLog('YsBLoq_prior', 'YsBLoq', np.log(p0[48]), np.log(np.sqrt(p0[48])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPHiq_prior', 'YsBPHiq', np.log(p0[49]), np.log(np.sqrt(p0[49])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPLoq_prior', 'YsBPLoq', np.log(p0[50]), np.log(np.sqrt(p0[50])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPP_prior', 'YsBPP', np.log(p0[51]), np.log(np.sqrt(p0[51])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFHiq_prior', 'YsFHiq', np.log(p0[52]), np.log(np.sqrt(p0[52])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsFLoq_prior', 'YsFLoq', np.log(p0[53]), np.log(np.sqrt(p0[53])))
m.AddResidual(res)
res = Residuals.PriorInLog('rB0_prior', 'rB0', np.log(p0[54]), np.log(np.sqrt(p0[54])))
m.AddResidual(res)
res = Residuals.PriorInLog('rBP0_prior', 'rBP0', np.log(p0[55]), np.log(np.sqrt(p0[55])))
m.AddResidual(res)
res = Residuals.PriorInLog('rF0_prior', 'rF0', np.log(p0[56]), np.log(np.sqrt(p0[56])))
m.AddResidual(res)
...
Utility.save(popt, 'popt2.model.bpkl')
