...
x = np.log(model33_fit.popt)

# Calculate jtj approximation to hessian (in log params) and plot eigenvalue
# spectrum
j = model33_fit.m.jacobian_log_params_sens(np.log(model33_fit.popt))
jtj = np.dot(np.transpose(j), j)
np.savetxt('hessian33.dat', jtj)
e, v = Utility.eig(jtj)
e = scipy.real(e)
Plotting.figure(1)
...
func = lambda logp: np.array(model33_fit.m.res_log_params(logp))
jacobian = lambda logp: np.array(model33_fit.m.jacobian_log_params_sens(logp).values())
...
# Calculate jtj approximation to hessian (in log params) and plot eigenvalue
# spectrum
j = model33_fit.m.jacobian_log_params_sens(geo.xs[-1,:])
jtj = np.dot(np.transpose(j), j)
e, v = Utility.eig(jtj)
e = scipy.real(e)
Plotting.figure(3)
...
plt.show()
print('corresponding to %d',model33_fit.popt.items()[ SDisc.items()[0][0]])

# Save parameter values at end of geodesic
p0 = np.exp(geo[-1,:])
Utility.save(p0, 'p33.geodesics.bpkl')
