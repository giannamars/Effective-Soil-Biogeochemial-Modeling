...
model_net = IO.from_SBML_file('PECCAT31.xml', 'base')
...
IO.eqns_TeX_file(model_net, 'model31.tex')

## Create the model
m = Model([PECCAT_experiment.expt], [model_net])
p0= m.get_params().copy()

p31 = Utility.load('p30.geodesics.bpkl')

p31 = np.delete(p31,28)
p31 = np.delete(p31,26)
p31 = np.delete(p31,25)
p31 = np.delete(p31,23)
p31 = np.delete(p31,4)
...
res = Residuals.PriorInLog('YLHiq_prior', 'YLHiq', np.log(p0[15]), np.log(np.sqrt(p0[15])))
m.AddResidual(res)
res = Residuals.PriorInLog('YLLoq_prior', 'YLLoq', np.log(p0[16]), np.log(np.sqrt(p0[16])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrB_prior', 'YrB', np.log(p0[17]), np.log(np.sqrt(p0[17])))
m.AddResidual(res)
res = Residuals.PriorInLog('YrF_prior', 'YrF', np.log(p0[18]), np.log(np.sqrt(p0[18])))
m.AddResidual(res)
res = Residuals.PriorInLog('YRFP_prior', 'YRFP', np.log(p0[19]), np.log(np.sqrt(p0[19])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBHiq_prior', 'YsBHiq', np.log(p0[20]), np.log(np.sqrt(p0[20])))
m.AddResidual(res)#
res = Residuals.PriorInLog('YsBLoq_prior', 'YsBLoq', np.log(p0[21]), np.log(np.sqrt(p0[21])))
m.AddResidual(res)
res = Residuals.PriorInLog('YsBPLoq_prior', 'YsBPLoq', np.log(p0[22]), np.log(np.sqrt(p0[22])))
m.AddResidual(res)
...
Utility.save(popt, 'popt31.model.bpkl')
