...
x = np.log(model41_fit.popt)

# Calculate jtj approximation to hessian (in log params) and plot eigenvalue
# spectrum
j = model41_fit.m.jacobian_log_params_sens(np.log(model41_fit.popt))
jtj = np.dot(np.transpose(j), j)
np.savetxt('hessian41.dat', jtj)
e, v = Utility.eig(jtj)
e = scipy.real(e)
Plotting.figure(1)
...
func = lambda logp: np.array(model41_fit.m.res_log_params(logp))
jacobian = lambda logp: np.array(model41_fit.m.jacobian_log_params_sens(logp).values())
...
# Calculate jtj approximation to hessian (in log params) and plot eigenvalue
# spectrum
j = model41_fit.m.jacobian_log_params_sens(geo.xs[-1,:])
jtj = np.dot(np.transpose(j), j)
e, v = Utility.eig(jtj)
e = scipy.real(e)
Plotting.figure(3)
...
plt.show()
print('corresponding to %d',model41_fit.popt.items()[ SDisc.items()[0][0]])

# Save parameter values at end of geodesic
p0 = np.exp(geo.xs[-1,:])
Utility.save(p0, 'p41.geodesics.bpkl')
