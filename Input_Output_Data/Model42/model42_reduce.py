...
x = np.log(model42_fit.popt)

# Calculate jtj approximation to hessian (in log params) and plot eigenvalue
# spectrum
j = model42_fit.m.jacobian_log_params_sens(np.log(model42_fit.popt))
jtj = np.dot(np.transpose(j), j)
np.savetxt('hessian42.dat', jtj)
e, v = Utility.eig(jtj)
e = scipy.real(e)
Plotting.figure(1)
...
func = lambda logp: np.array(model42_fit.m.res_log_params(logp))
jacobian = lambda logp: np.array(model42_fit.m.jacobian_log_params_sens(logp).values())
...
plt.show()
print('corresponding to %d',model42_fit.popt.items()[ SDisc.items()[0][0]])

# Save parameter values at end of geodesic
p0 = np.exp(geo.xs[-1,:])
Utility.save(p0, 'p42.geodesics.bpkl')
